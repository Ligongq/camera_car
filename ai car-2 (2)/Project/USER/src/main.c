...
int16 smotor_angle,smotor_angle1;			//舵机控制变量
...
{
   static int i=0,zuo=0,you=0;// 左右计数  
...
//    gpio_init(B11, GPO, 0, GPIO_PIN_CONFIG); //蜂鸣器
//    gpio_init(C25,GPI,1,FAST_GPIO_PIN_CONFIG);//按键
...
//    gpio_init(B13,GPI,1,FAST_GPIO_PIN_CONFIG);//干簧管
...
void smotor_init()
{
	...
	pwm_init(PWM1_MODULE3_CHA_D0, 13000, 0); //右
	pwm_init(PWM1_MODULE3_CHB_D1, 13000, 0); //左
	gpio_set(D2, 1);						 //左
	gpio_set(D3, 1);						 //右
}
void Opa_Init()
...
void fenqvtiaosu()
{
	if((duojijiaodu>(SMOTOR_CENTER+80))||(duojijiaodu<(SMOTOR_CENTER-80)))  //小弯
	{
//		if(shi>=17&&shi<=20)
//		{	
//			y=0.5*wdsd-25;
...
int main(void)
{
	...
	smotor_init();
	bian_ma_qi();	
	//gpio_interrupt_init(D27,FALLING,GPIO_INT_CONFIG);	//外部中断干簧管
	//NVIC_SetPriority(GPIO3_Combined_16_31_IRQn,0);	    //设置外部中断优先级
	pwm_init(PWM4_MODULE3_CHA_C31, 50, SMOTOR_CENTER);
    ...
    while(1)
    {
		if(collection_succes)
		{
			...
			cie_data_get();//获取RunModel函数所需的参数
				...
				smotor_angle = g_servoValue * (int32)420 / 128;//放大数据
			}
    }
}
